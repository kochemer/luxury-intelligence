  ...
  return (
    <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-16 py-16 md:py-20">
      <header className="max-w-2xl mb-12 md:mb-16">
        <Link href="/da" className="text-[var(--color-accent)] hover:text-[var(--color-text-primary)] text-meta inline-block mb-6 transition-colors">
          ← Tilbage til start
        </Link>
        ...
        <p className="text-body text-[var(--color-text-secondary)]">
          {weekLabels.length} udgaver · AI, ecommerce, luksus &amp; smykkebranchen
        </p>
      ...
      {issues.length > 0 ? (
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-3 md:p-4">
                ...
                {meta.totalArticles > 0 && (
                  <p className="text-[11px] text-white/50 font-sans mt-1">{meta.totalArticles} artikler</p>
                )}
              </div>
            </Link>
          ))}
        ...
        <div className="bg-[var(--color-accent-light)] border-l-4 border-[var(--color-accent)] p-5 rounded-sm">
          ...
          <p className="text-body text-[var(--color-text-secondary)]">
            Kør <code className="bg-[var(--color-surface)] px-2 py-1 rounded text-meta font-mono border border-[var(--color-border)]">npx tsx scripts/buildWeeklyDigest.ts</code> for at oprette oversigter.
          </p>
        </div>
      )}
    </div>
